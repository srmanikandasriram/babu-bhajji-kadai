...
void First_LineFollow(){
  ...
  int temp_flag = 0, flicker_flag = 0; 

  while(1){
    LineFollow34();
    if(temp_flag==0 && encoder_turret > encoder_turret_target )
      turret_motor.Brake(255);
    Move_Servo();
    
    if(temp_flag == 0 && analogRead(SHARP_SENSOR_PIN_2) > 350) {//correct it later and put flicker correction
      flicker_flag += 1;
      if (flicker_flag > 0) {
        temp_flag = 1;
        Motors_Brake(255,255);
        ...
        Toggle_Wait();
        LAPTOP.println("Dropping first leaf");
        Actuate_High(MIDDLE_VG); 
        delay(500);
        //encoder_turret_target = TURRET_ANG2BF;
      
        turret_motor.Control(Check_Mirror(BCK,FWD), 250);
      }
    } else if(temp_flag == 0) {
      flicker_flag = 0;
    }
    ...
    if(turret_sensor.Low())
      turret_motor.Brake(255);
    if(temp_flag != 0 && ( S4.High() && S2.High() )||( S1.High() && S3.High() ))
      break;
  }
}
...
void To_Last_Leaf(){
  ...
  LAPTOP.println("reached junction");
  Serial_Print_Sensors();
  fl0,loat flicker_flag = 0, flicker_value = 0;
  int threshold = Check_Mirror(300, 125);
  while(1)
    {
      LineFollow_Straight_slow();
      if(encoder_turret > encoder_turret_target )
        turret_motor.Brake(255);
      //Move_TurretF();
      flicker_value += analogRead(SHARP_SENSOR_PIN);
      flicker_flag += 1;
      if(flicker_flag > 10) {
        float avg = flicker_value/flicker_flag;
        flicker_flag = 0;
        flicker_value = 0;
        LAPTOP.println(avg);
        if(avg > threshold) // check for mirror later
          {
            
            Motors_Brake(255,255);
            LAPTOP.println("Wait for turret again");
            while(encoder_turret <= encoder_turret_target );
            turret_motor.Brake(255);
            break;
  ...
  Parameters_Reset();
  int threshold = Check_Mirror(300, 350); // NEED TO CHANGE
...
LAPTOP.println("Dropping Third Leaf");
  ...
  while((!( S4.High() && S3.High() && S2.High() ))|| (!( S1.High() && S2.High() && S3.High()  )));
  ...
  Motors_Brake(255,255);
  delay(200);
  //Toggle_Wait();atest change on 23rd feb delay was there before 400
